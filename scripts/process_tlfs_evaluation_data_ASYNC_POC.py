...
UPLOAD_ROWS = 16
SIZE_MINIBATCH = 8
SIZE_ASYNC_MINIBATCH = 4
RESPONSE_TIMEOUT = 20 # max wait-time (s) for a response
...
def process_row_minibatch(rows, secret_code, app_config):
    """Process a minibatch of ~6 rows of the DataFrame, make async API requests, and return the responses.
    If an error occours (404, 503, etc), the UID, payload and error are returned instead.

    Parameters:
    rows list(pd.Series): A list of rows from the DataFrame.
    secret_code (str): The secret code for API authorization.
    app_config : the loaded configuration toml.

    Returns:
    list(dict): list of response JSON objects with additional information.
    """
    class dummy_response:
        ...
        def raise_for_status(self):
            raise requests.exceptions.RequestException(self.exception)

    def _exception_handler(request, exception):
        return dummy_response(request, exception)

    base_url = (
        os.getenv("API_GATEWAY", "http://127.0.0.1:5000") + "/survey-assist/classify"
    )
    unique_ids = [row[app_config["column_names"]["payload_unique_id"]] for row in rows]
    job_titles = [row[app_config["column_names"]["payload_job_title"]] for row in rows]
    job_descriptions = [row[app_config["column_names"]["payload_job_description"]] for row in rows]
    industry_descriptions = [row[app_config["column_names"]["payload_industry_description"]] for row in rows]
    ...
    payloads = [{
        ...
        "job_description": job_description,
        "industry_descr": industry_description,
    } for job_title, job_description, industry_description in zip(job_titles, job_descriptions, industry_descriptions)]

    responses_setup = (grequests.post(base_url, 
                                      headers=headers, 
                                      data=json.dumps(payload), 
                                      timeout=RESPONSE_TIMEOUT
    ) for payload in payloads)
    responses = grequests.map(responses_setup, 
                              size=SIZE_ASYNC_MINIBATCH,
                              exception_handler = _exception_handler)
    responses_json = []
    for uid,response,payload in zip(unique_ids,responses,payloads):
        try:
            response.raise_for_status()
            response_json = response.json()
            response_json.update({"unique_id": uid, "request_payload": payload})
            responses_json.append(response_json)
        except requests.exceptions.RequestException as e:
            logging.error("Request failed for unique_id %s: %s", uid, e)
            response_json = {
                "unique_id": uid,
                "request_payload": payload,
                "error": str(e),
    ...
    with open(local_output_path, "w+", encoding="utf-8") as target_file:
        ...
        for i, (_index, row) in enumerate(process_batch_data.iterrows()):
            ...
            if (i+1)%SIZE_MINIBATCH != 0:
                ...
                if is_gcs_output and ((i + 1) % UPLOAD_ROWS < SIZE_MINIBATCH or (i + 1) == total_rows):
                    logging.info(
