...
# %%
cols_wanted = ['unique_id', 'sic_ind_occ1',
       'sic_ind_occ2', 'sic_ind_occ3', 'sic_ind_occ_flag', 'Not_Codeable',
       'Four_Or_More', 'SIC_Division', 'num_answers',
       'All_Clerical_codes', 'Match_5_digits', 'Match_3_digits',
       'Match_2_digits', 'Unambiguous']

# Load your DataFrame
eval_data = pd.read_csv("../data/analysis_outputs/TLFS_evaluation_data_IT2_output.csv",usecols=cols_wanted, dtype={"SIC_Division": str})
...
plot_sic_code_histogram(eval_data, column_name="SIC_Division", output_dir = output_dir_path, show_percent = True , filename_suffix = "SIC_Division")

# %%
# Calculate membership of 86 and 87  combined
print(f"""Fraction of the dataset being in Division 86 or 87:
...
# %%
combined_data = pd.concat([eval_data['sic_ind_occ1'], eval_data['sic_ind_occ2'], eval_data['sic_ind_occ3']])
combined_df = pd.DataFrame(combined_data, columns=['combined'])
plot_sic_code_histogram(combined_df, column_name="combined", output_dir = output_dir_path, show_percent = True , filename_suffix = "Combined_sic_codes")
...
plot_sic_code_histogram(filtered_data, column_name="sic_ind_occ1", output_dir = output_dir_path, show_percent = True , filename_suffix = "Unambiguous_only")
...
# %%
plot_sic_code_histogram(eval_data, column_name="num_answers", output_dir = output_dir_path, show_percent = True , filename_suffix = "")

# %%
print(f"""Single answer given {100*(eval_data["num_answers"] == 1).sum() / 
...
print(f"""Uncodeable to any digits: {100*(eval_data["num_answers"] == 0).sum() / 
        len(eval_data["num_answers"]):.2f} %""")
print(f"""Codeable to two digits only:  {100*eval_data['Match_2_digits'].sum() / 
        len(eval_data):.1f} %""")
print(f"""Codeable to five Digits (ambiguous and unambiguous):
        {100*eval_data['Match_5_digits'].sum() /len(eval_data):.1f} %""")
print(f"""Codeable unambiguously to five digits:
        {100*eval_data['Unambiguous'].sum() / len(eval_data["Match_5_digits"]):.1f} %""")

ambiguous_data = eval_data[~eval_data['Unambiguous']]
print(f"""Codeable ambiguously to five digits:
...
print(f"Fraction of True 'Match_5_digits': {eval_data['Match_5_digits'].sum() /len(eval_data):.1f}")

print(f"Number of True in 'Match_2_digits': {eval_data['Match_2_digits'].sum()}")
print(f"Fraction of True 'Match_2_digits': {eval_data['Match_2_digits'].sum() /len(eval_data):.2f}")

filtered_data = eval_data[eval_data['Match_5_digits'] | eval_data['Match_3_digits'] | eval_data['Match_2_digits']]
true_count_2_or_more = len(filtered_data)
fraction_2_or_more = true_count_2_or_more / len(eval_data)
print(f"""Number of True values in 'Match_2_digits',
 'Match_3_digits' or 'Match_5_digits': {true_count_2_or_more}""")
print(f"Fraction of True values in 'Match_2_digits' or more digits: {fraction_2_or_more:.2f}")
...
#codeable at 2d
filtered_data = eval_data[eval_data['Match_5_digits'] | eval_data['Match_3_digits'] | eval_data['Match_2_digits']]
plot_sic_code_histogram(filtered_data, column_name="sic_ind_occ1",  output_dir = output_dir_path, show_percent = True , filename_suffix = "Codeable")
